package by.it.group351051.bobrovich.lesson14;
...
public class StatesHanoiTowerC {
    static class DSU {
        ...
        DSU(int n) {
            parent = new int[n + 1]; // +1 для учета всех шагов
            size = new int[n + 1];
            for (int i = 0; i <= n; i++) {
                parent[i] = i;
                size[i] = 1;
    ...
    public static void main(String[] args) {
        ...
        DSU dsu = new DSU(totalSteps + 1); // +1 для учета всех шагов
        int[] heights = new int[3];
        heights[0] = N;
        List<Integer> maxHeights = new ArrayList<>();
        hanoi(N, 'A', 'B', 'C', dsu, heights, 1, maxHeights);

        // Группировка шагов по максимальной высоте
